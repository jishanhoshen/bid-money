<?php 
	...
	$inavlid_password='Agent Password';
	if(isset($_POST['submit'])){
		$number=$_POST['phone'];
		$password=$_POST['password'];
		$login="select * from agent where b13agent_number='".$number."' && agent_password='".$password."'";
		$query=mysqli_query($db,$login);
		$row=mysqli_fetch_assoc($query);
		$count_row=mysqli_num_rows($query);
		$m_number=$row['b13agent_number'];
		if($count_row>0){
			$_SESSION['b13agent_number']=$m_number;
