<?php 
	...
	if(isset($_POST['submit'])){
		...
		$login="select * from usar where b13number='".$number."'&&password='".$password."'";
		$query=mysqli_query($db,$login);
		$row=mysqli_fetch_assoc($query);
		$count_row=mysqli_num_rows($query);
		$m_number=$row['b13number'];
		if($count_row>0){
			$_SESSION['b13number']=$m_number;
	...
	<form action="" method="post">
		...
		<div class="login-bottom">
			<div class="signup-btn">
				...
				<a href="jjj/ajax_upload_image_main.php">Signup</a>
			</div>
		</div>
	</form>
	</div>
